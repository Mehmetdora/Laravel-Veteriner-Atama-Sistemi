    ...
    <div class="content-wrapper">
        ...
        <div class="content-header">
            <div class="container-fluid">
                <div class="row mb-2">
                    <div class="col-sm-6">
                        <h1 class="m-0"><b>Ürün Kategorileri</b></h1>
                    </div><!-- /.col -->
                </div><!-- /.row -->
            </div><!-- /.container-fluid -->
        </div>
                                    ...
                                    <tbody>
                                        @if (isset($uruns))
                                            @foreach ($uruns as $urun)
                                                <tr>
                                                    <th style="width:50%">{{ $urun->name }}</th>
                                                    <td>
                                                        <a
                                                            href="{{ route('admin.uruns.edit', $urun->id) }}">Düzenle</a>
                                                        |
                                                        <a data-toggle="modal" data-target="#modal-evrak-delete"
                                                            role="button" class="evrakT_sil"
                                                            data-yeni="{{ route('admin.uruns.delete', $urun->id) }}"
                                                            data-id="{{ $urun->id }}">
                                                            Sil
                                                        </a>
                                                    </td>
